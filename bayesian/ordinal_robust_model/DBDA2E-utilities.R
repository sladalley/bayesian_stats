# Adapted version of Kruschke's script.
...
plotPost = function( paramSampleVec , cenTend=c("mode","median","mean") , 
  ...
  if ( "median" %in% cenTend ){ 
    text( med , cenTendHt ,
          bquote(md==.(signif(med,3))) , adj=c(.5,-1.25) , cex=cex , col=cvCol )
  }
  if ( "mean" %in% cenTend ){ 
    text( mn , cenTendHt ,
  ...
  if ( !is.null( ROPE ) ) {
    ...
    lines( c(ROPE[1],ROPE[2]) , c(1.1*ROPEtextHt,1.1*ROPEtextHt) , lwd=2 , 
           col=ropeCol) 
    lines( c(ROPE[1],ROPE[1]) , c(1.07*ROPEtextHt,1.13*ROPEtextHt) , lwd=2 , 
           col=ropeCol)
    lines( c(ROPE[2],ROPE[2]) , c(1.07*ROPEtextHt,1.13*ROPEtextHt) , lwd=2 , 
           col=ropeCol)
    text( mean(ROPE) , 1.2*ROPEtextHt , 
    ...
    lines( c(ROPE[2],ROPE[2] + arrow_len) , 
           c(0.96*ROPEtextHt,0.96*ROPEtextHt) , lwd=2 , 
           col=ropeCol)
    text ( ROPE[2] + arrow_len , 1.06*ROPEtextHt , 
