...
# ====== MCMC chain ===========================================================
genMCMC_lnorm = function(datFrm, yName="y", gName="cond", muPrior=NULL, muSdPrior=NULL, 
  ...
  #   - sigmaPriorMean: mean of the prior for the mean of each group (normal dist.).
  #   - muSdPrior: std. dev. of the prior for the mean of each group (normal dist.).
  #   - sigmaPriorLow: low limit of the prior for the mu of each group (uniform dist.).
  #   - sigmaPriorHigh: high limit of the prior for the mu of each group (uniform dist.).
  ...
  model {
    for (i in 1:Ntotal) {
      ...
      y[i] ~ dlnorm(mu[g[i]], 1/sigma[g[i]]^2)
    }

    for (gIdx in 1:nG) {
        mu[gIdx] ~ dnorm( muPrior[gIdx] , 1/(muSdPrior[gIdx])^2 )
       sigma[gIdx] ~ dunif( sigmaPriorLow[gIdx] , sigmaPriorHigh[gIdx] )
      # LOCATION (log-space mean)
      mu[gIdx] ~ dnorm(0.0, 1.0E-6)
  
      # SCALE (Jeffreys-consistent: 1/sigma behaviour via log transform)
    #  log_sigma[gIdx] ~ dnorm(0.0, 1.0)
      #sigma[gIdx] <- exp(log_sigma[gIdx])
  
     # tau[gIdx] <- 1 / (sigma[gIdx] * sigma[gIdx])
  ...
  #   The estimated paremeters are: sigma, mu.
  #   The initial mu and sigma. are the ones obtained from the data and
  ...
  #-----------------------------------------------------------------------------
  # RUN THE CHAINS
  parameters = c("mu", "sigma") ## <- the parameters to be monitered
  numSavedSteps = 20000
  thinSteps = 5
...
smryMCMC_lnorm = function (codaSamples, nG, nullValEff=0, saveName=NULL, 
...
plotMCMC_lnorm = function( codaSamples, datFrm, yName="y", gName="cond", 
  ...
  if (nG == 1) {
    ...
    muLim = range( mcmcMat[, grep("^sigma",colnames(mcmcMat)) ] )
    plotPost( mcmcMat[,"sigma"] , xlab=TeX(sprintf("sigma")) , xlim=sigmaLim , 
  ...
  if (nG == 2) {
    ...
    plotPost( mcmcMat[,"sigma[1]"] , 
              xlab=TeX(sprintf("sigma %s", groupNames[1])) , 
              xlim=sigmaLim  , 
    ...
    plotPost( mcmcMat[,"sigma[2]"] , 
              xlab=TeX(sprintf("sigma %s", groupNames[2])) , 
              xlim=sigmaLim  , 
              col="#0088aa8a" , cex=cex_plotPost )
  ...
  saveGraph( paste0(saveName,"-sigmamu") , type=graphFileType )
  ...
  for ( gIdx in 1:nG ) {
    ...
    if (nG == 1) {
      for ( stepIdx in 1:length(stepIdxVec) ) {
        ...
        sigma = mcmcMat[stepIdx, "sigma"]
        mu = mcmcMat[stepIdx, "mu"]
        
        
        dens = dlnorm(xVec-40, mu, sigma)
    ...
    if (nG > 1) {
      for ( stepIdx in 1:length(stepIdxVec) ) {
        xVec = seq( xLim[1] , xLim[2] , length=501 )
        sigma = mcmcMat[stepIdxVec[stepIdx], paste0("sigma[", gIdx, "]")]
        mu = mcmcMat[stepIdxVec[stepIdx], paste0("mu[", gIdx, "]")]
        
        #dens  = sigma * mu * xVec^(sigma - 1) * exp(-mu * xVec^sigma)
        dens = dlnorm(xVec, mu, sigma)
        lines(xVec, dens, type="l" , col="#0088aa8a" , lwd=1 )
      }
    }
  }
  ...
  if (nG == 1) {
    ...
    pairs( cbind( mcmcMat[,"sigma"] , mcmcMat[,"mu"] )[plotIdx,] ,
           labels=c( TeX(sprintf("sigma")) , 
                     TeX(sprintf("mu"))),
  ...
  if (nG == 2) {
    ...
    nPtToPlot = 1000
    plotIdx = floor(seq(1,length(mcmcMat[,"sigma[1]"]),
                        by=length(mcmcMat[,"sigma[1]"])/nPtToPlot))
    panel.cor = function(x, y, digits=2, prefix="", cex.cor, ...) {
      usr = par("usr"); on.exit(par(usr))
      par(usr = c(0, 1, 0, 1))
      r = (cor(x, y))
    ...
    pairs( cbind( mcmcMat[,"sigma[1]"] , mcmcMat[,"sigma[2]"] , 
                  mcmcMat[,"mu[1]"], mcmcMat[,"mu[2]"])[plotIdx,] ,
           labels=c( TeX(sprintf("sigma %s", groupNames[1])) , 
                     TeX(sprintf("sigma %s", groupNames[2])) , 
                     TeX(sprintf("mu %s", groupNames[1])) , 
                     TeX(sprintf("mu %s", groupNames[2]))),
           lower.panel=panel.cor , col="#0088aa8a" )
...
goalAchievedForSample = function( data, muNULL, effROPE, effHDImaxWid,
  ...
  mcmcMatEff = ( mcmcMat[,"sigma"] - muNULL ) / mcmcMat[,"sigma"]
...
powerEstimation = function( mcmcChain, N, muNULL, 
  ...
  for ( goalIdx in 1:NCOL(goalTally) ) {
    ...
    goalEstHDI = HDIofICDF( qbeta ,
                            sigma1=1+goalHits , 
                            sigma2=1+goalAttempts-goalHits )
    # Display the result:
