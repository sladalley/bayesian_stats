...
# ====== MCMC chain ===========================================================
genMCMC_weibull = function(datFrm, yName="y", gName="cond",
                           shapePriorMean = NULL, shapePriorSD = NULL,
                           scalePriorLow = NULL, scalePriorHigh = NULL, numSavedSteps = 20000,
                           saveName=NULL) {
  ...
  #   - shapePriorMean: mean of the prior for the mean of each group (normal dist.).
  ...
  if (nG == 2){
    if (is.null(shapePriorMean))
    {
      shapePriorMean = c( mean(y[ g==1 ]), mean(y[ g==2 ]) )
    }
    if (is.null(shapePriorSD))
    {
      shapePriorSD = c( sd(y[ g==1 ]), sd(y[ g==2 ]) )
    }
    if (is.null(scalePriorLow))
    {
      scalePriorLow = c( 0, 0 )
    }
    if (is.null(scalePriorHigh))
    {
      scalePriorHigh = 10*c( sd(y[ g==1 ]), sd(y[ g==2 ]) ) 
    }
  }
  ...
  dataList = list(
    ...
    nG = nG,
    Ntotal = Ntotal,
    shapePriorMean = shapePriorMean,
    shapePriorSD = shapePriorSD,
    scalePriorLow = scalePriorLow,
    scalePriorHigh = scalePriorHigh
  )
  #-----------------------------------------------------------------------------
  # THE METRIC WEIBELL MODEL.
  modelString = "
  model {
    for (i in 1:Ntotal) {
      y[i] ~ dweib( shape[g[i]] , rate[g[i]])
    }

    for (gIdx in 1:nG) {
      shape[gIdx] ~ dgamma(2, 2)
      scale[gIdx] ~ dunif( scalePriorLow[gIdx] , scalePriorHigh[gIdx] )
      rate [gIdx] <- (1/(scale[gIdx])^shape[gIdx])
  ...
  #   The estimated paremeters are: shape, scale.
  #   The initial scale and shape. are the ones obtained from the data and
  ...
  if (nG == 2) {
    initsList = list(
      shape = rep(1, nG),
      scale = c( (mean(y[g==1])) , (mean(y[g==2]))) 
    )
  }
  
  #-----------------------------------------------------------------------------
  # RUN THE CHAINS
  parameters = c("shape", "scale") ## <- the parameters to be monitered
  thinSteps = 5
...
smryMCMC_weibull = function (codaSamples, nG, nullValEff=0, saveName=NULL, 
  ...
  if (nG == 1) {
    ...
    summaryInfo = rbind(summaryInfo,
                        "scale" = summarizePost(mcmcMat[,"scale"])
    )
    
    summaryInfo = rbind(summaryInfo,
                        "shape" = summarizePost(mcmcMat[,"shape"])
    )
  ...
  if (nG == 2) {
    ...
    summaryInfo = rbind( summaryInfo , 
                         "scale[1]" = summarizePost( mcmcMat[,"scale[1]"] , 
                                                     compVal=NULL , ROPE=NULL ) )
    # Scale parameter of group 1:
    summaryInfo = rbind( summaryInfo , 
                         "scale[2]" = summarizePost( mcmcMat[,"scale[2]"] , 
                                                      compVal=NULL , ROPE=NULL ) )
    
    # Scale parameter of group 1:
    summaryInfo = rbind( summaryInfo , 
                         "shape[1]" = summarizePost( mcmcMat[,"shape[1]"] , 
                                                      compVal=NULL , ROPE=NULL ) )
    
    # Scale parameter of group 1:
    summaryInfo = rbind( summaryInfo , 
                         "shape[2]" = summarizePost( mcmcMat[,"shape[2]"] , 
                                                      compVal=NULL , ROPE=NULL ) )
...
plotMCMC_weibull = function( codaSamples, datFrm, yName="y", gName="cond", 
  ...
  if (nG == 1) {
    ...
    shapeLim = range( mcmcMat[, grep("^shape",colnames(mcmcMat)) ] )
    scaleLim = range( mcmcMat[, grep("^sigma",colnames(mcmcMat)) ] )
    plotPost( mcmcMat[,"shape"] , xlab=TeX(sprintf("shape")) , xlim=shapeLim , 
                compVal=compValMu , cex=cex_plotPost , col="#0088aa8a")
    plotPost( mcmcMat[,"scale"] , xlab=TeX(sprintf("scale")) , xlim=scaleLim , 
  ...
  if (nG == 2) {
    ...
    shapeLim = range( mcmcMat[, grep("^shape",colnames(mcmcMat)) ] )
    scaleLim = range( mcmcMat[, grep("^scale",colnames(mcmcMat)) ] )
    
    #print(shapeLim)
    #print(scaleLim)

      plotPost( mcmcMat[,"shape[1]"] , 
                xlab=TeX(sprintf("shape %s", groupNames[1])) , 
                xlim=shapeLim  , 
                col="#0088aa8a" , cex=cex_plotPost )
      plotPost( mcmcMat[,"scale[1]"] , 
                xlab=TeX(sprintf("scale %s", groupNames[1])) , 
                xlim=scaleLim , 
                col="#0088aa8a" , cex=cex_plotPost )
      plotPost( mcmcMat[,"shape[2]"] , 
                xlab=TeX(sprintf("shape %s", groupNames[2])) , 
                xlim=shapeLim  , 
                col="#0088aa8a" , cex=cex_plotPost )
      plotPost( mcmcMat[,"scale[2]"] , 
                xlab=TeX(sprintf("scale %s",  groupNames[2])) , 
                xlim=scaleLim , 
                col="#0088aa8a" , cex=cex_plotPost )
  } 
  saveGraph( paste0(saveName,"-ShapeScale") , type=graphFileType )
  ## Mean
  if (nG == 2) {
    # Two groups:
    openGraph(height=2.5*(nG+1),width=7)
    ...
    plotPost( mcmcMat[,"scale[1]"]*gamma(1+1/mcmcMat[,"shape[1]"]) , 
              xlab=TeX(sprintf("$\\mu_{%s}$", groupNames[1])) , 
              col="#0088aa8a" , cex=cex_plotPost )
    plotPost( mcmcMat[,"scale[2]"]*gamma(1+1/mcmcMat[,"shape[2]"]) , 
              xlab=TeX(sprintf("$\\mu_{%s}$", groupNames[2])) , 
              col="#0088aa8a" , cex=cex_plotPost )
    plotPost( (mcmcMat[,"scale[1]"]*gamma(1+1/mcmcMat[,"shape[1]"]))- (mcmcMat[,"scale[2]"]*gamma(1+1/mcmcMat[,"shape[2]"])) , ROPE=ropeMu,
              xlab=TeX(sprintf("$\\mu_{%s} - \\mu_{%s}$", groupNames[1], groupNames[2])) , 
              col="#0088aa8a" , cex=cex_plotPost )

  } 
  saveGraph( paste0(saveName,"-Mean") , type=graphFileType )
  ...
  for ( gIdx in 1:nG ) {
    ...
    if (nG == 1) {
      for ( stepIdx in 1:length(stepIdxVec) ) {
        ...
        shape = mcmcMat[stepIdx, "shape"]
        scale = mcmcMat[stepIdx, "scale"]
        
        
        dens  = shape * scale * xVec^(shape - 1) * exp(-scale * xVec^shape)
    ...
    if (nG > 1) {
      for ( stepIdx in 1:length(stepIdxVec) ) {
        xVec = seq( xLim[1] , xLim[2] , length=501 )
        shape = mcmcMat[stepIdxVec[stepIdx], paste0("shape[", gIdx, "]")]
        scale = mcmcMat[stepIdxVec[stepIdx], paste0("scale[", gIdx, "]")]
        
        #dens  = shape * scale * xVec^(shape - 1) * exp(-scale * xVec^shape)
        dens = dweibull(xVec, shape, scale)
        lines(xVec, dens, type="l" , col="#0088aa8a" , lwd=1 )
      }
    }
  }
  ...
  if (nG == 1) {
    ...
    panel.cor = function(x, y, digits=2, prefix="", cex.cor, ...) {
      ...
      pairs( cbind( mcmcMat[,"shape"] , mcmcMat[,"scale"] )[plotIdx,] ,
             labels=c( TeX(sprintf("shape")) , 
                       TeX(sprintf("scale"))),
  ...
  if (nG == 2) {
    ...
    nPtToPlot = 1000
    plotIdx = floor(seq(1,length(mcmcMat[,"shape[1]"]),
                        by=length(mcmcMat[,"shape[1]"])/nPtToPlot))
    panel.cor = function(x, y, digits=2, prefix="", cex.cor, ...) {
      usr = par("usr"); on.exit(par(usr))
      par(usr = c(0, 1, 0, 1))
      r = (cor(x, y))
      ...
      pairs( cbind( mcmcMat[,"shape[1]"] , mcmcMat[,"shape[2]"] , 
                    mcmcMat[,"scale[1]"], mcmcMat[,"scale[2]"])[plotIdx,] ,
             labels=c( TeX(sprintf("shape %s", groupNames[1])) , 
                       TeX(sprintf("shape %s", groupNames[2])) , 
                       TeX(sprintf("scale %s", groupNames[1])) , 
                       TeX(sprintf("scale %s", groupNames[2]))),
             lower.panel=panel.cor , col="#0088aa8a" )
...
goalAchievedForSample = function( data, muNULL, effROPE, effHDImaxWid, muROPE,
  ...
  mcmcCoda = genMCMC_weibull( datFrm = data , numSavedSteps=mcmcLength , saveName=NULL, yName = yName, gName = gName )
  mcmcMat = as.matrix(mcmcCoda,chains=TRUE)
  # Calculate effect size:
  HDImat = apply( mcmcMat , 2 , "HDIofMCMC" )
  mcmcMatDM = (mcmcMat[,"scale[1]"]*gamma(1+1/mcmcMat[,"shape[1]"]))- (mcmcMat[,"scale[2]"]*gamma(1+1/mcmcMat[,"shape[2]"]))
  mcmcMatMu1 = mcmcMat[,"scale[1]"]*gamma(1+1/mcmcMat[,"shape[1]"])
  mcmcMatMu2 = mcmcMat[,"scale[2]"]*gamma(1+1/mcmcMat[,"shape[2]"])
  ...
  goalAchieved = c( goalAchieved , 
                    "mu1NarrowHDI"= all( effMu1[2] - effMu1[1]
                                        < HDImaxWid ) )
  
  goalAchieved = c( goalAchieved , 
                    "mu2NarrowHDI"= all( effMu2[2] - effMu2[1]
                                         < HDImaxWid ) )
...
powerEstimation_weibull = function( mcmcChain, N, muNULL, muROPE,
  ...
  while (nSim < length(stepIdxVec) ) {
    ...
    shape1 = mcmcChain[stepIdx,"shape[1]"]
    shape2 = mcmcChain[stepIdx,"shape[2]"]
    
    scale1 = mcmcChain[stepIdx,"scale[1]"]
    scale2 = mcmcChain[stepIdx,"scale[2]"]
    
    
    # Simulate data for each controller
    y1 = rweibull(N1, shape1, scale1)
    y2 = rweibull(N2, shape2, scale2)
