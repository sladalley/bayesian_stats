...
genMCMC_beta = function(datFrm, yName="y", gName="cond", muAlphaPrior = NULL, muBetaPrior = NULL, 
  ...
  s = as.numeric(as.factor(datFrm[,sName])) # group scene numbers
  sLevels = levels(as.factor(datFrm[,sName]))
  nS = max(s) # number of groups
  ...
  model {
    ...
    for (i in 1:Ntotal) {
      ...
      alpha[i] <- mu[s[i], g[i]]*kappa[s[i]]
      beta[i] <- (1-mu[s[i],g[i]])*kappa[s[i]]
    }
  
    for (sIdx in 1:nS) {
      for (gIdx in 1:nG) {
        mu[sIdx, gIdx] ~ dbeta(alpha_mu, beta_mu)

      }
    }
    for (sIdx in 1:nS) {
       kappa[sIdx] ~ dgamma(a_kappa, b_kappa)
